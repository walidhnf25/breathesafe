...
LARAVEL_LOCAL_API = "http://localhost:8000"
...
def save_detection_to_laravel(image_array, camera_id=1):
    ...
    try:
        ...
        response = requests.post(LARAVEL_API_URL+"/api/detection-images", files=files, data=data, timeout=10)
...
def process_live_stream(camera_id, video_path):
    ...
    while stream_active:
        ...
        # Update latest frame for specific camera
        if camera_id == 1:
        ...
        if frame_count % STREAM_FRAME_SKIP == 0:
            try:
                ...
                # If smoke detected, save to API and add to detection history
                ...
                print(f"❌ Error processing live frame {frame_count} (Camera {camera_id}): {e}")
